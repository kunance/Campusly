...
=======
Any ENUM model needs to have the double quotes removed
Generated by the model script: type: Seq.ENUM('"no preference"', '"male only"', '"female only"')
Required: type: Seq.ENUM('no preference', 'male only', 'female only')
